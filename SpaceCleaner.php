...
class SpaceCleaner
{
	...
	public function start()
	{
		...
		$freeSpace = $this->getFreeSpace();
		$freeSpace = round($freeSpace/1024/1024/1024);
		$this->message("Свободного места на сервере: {$freeSpace} GB");

		if ($this->getFreeSpace() < $this->minimumFreeSpace) {
			sendEmailsNotifocation("На сервере не достаточно места. Осталось {$freeSpace} GB, необходимо ".round($this->minimumFreeSpace/1024/1024/1024)." GB");
		}
